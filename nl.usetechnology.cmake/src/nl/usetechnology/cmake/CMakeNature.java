...
public class CMakeNature implements IProjectNature {
	...
	private static final StringBuilder makefileContent = new StringBuilder();
	
	static {
		makefileContent.append("# This file has been generated, do not edit this file.\n");
		makefileContent.append("# This makefile is the \"gateway\" for the CMake build system.\n");
		makefileContent.append("# This is only required for the eclipse build.\n\n");
		makefileContent.append(".DEFAULT:\n");
		makefileContent.append("\t$(MAKE) -C bin/${USE_ARCH} $(@)\n");
	}
	
	private static void scheduleGenerateMakefile(final IProject project) {
		WorkspaceJob job = new WorkspaceJob("Generate Makefile for project " + project) {
			@Override
			public IStatus runInWorkspace(IProgressMonitor monitor) throws CoreException {
				project.refreshLocal(IResource.DEPTH_ONE, monitor);
				IFile file = project.getFile("Makefile");
				try {
					FileContentIO.writeFileContent(file, makefileContent, monitor);
				} catch (IOException e) {
					e.printStackTrace();
				}
		...
		WorkspaceJob job = new WorkspaceJob("Check integrity of project " + project.getName()) {
			...
			public IStatus runInWorkspace(IProgressMonitor monitor)
				...
				try {
					...
					if(!project.getFile("Makefile").exists()) {
						scheduleGenerateMakefile(project);
					}
